...
!!! tip "Orin Nano Super"
    ...
    Mechanical assembly matches classic JetBot. Use the **Orin Nano Developer Kit** on the chassis instead of Jetson Nano. Prefer **NVMe SSD** under the module. Motor I2C is on **bus 1**; many Orin expansion boards use motor address **0x70** — see [Orin I2C](reference/i2c_orin.md). **Power** for Super mode is not the upstream light PD pack — see [Power](power.md).

    Official photo walkthrough: [jetbot.org Hardware Setup](https://jetbot.org/master/hardware_setup.html).
