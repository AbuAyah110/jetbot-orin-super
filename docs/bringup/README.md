...
| F | **Compact F4/F5 replacement passes — only F6 open** | F1 name-resolved ALSA mixer. F2 `pywebrtc-audio` offline NS 8.2× / AEC 236×. **F3 verdict: reject RNNoise**. Current default: Zipformer small int8 ASR + Piper Lessac-low int8 VITS in one `sherpa-onnx` CPU process, WER 0.00, 162.3 MiB peak, zero VRAM. Historical FastConformer/Matcha evidence remains in the notes. Live mic/playback was not used for the replacement gate. |
