...
* **Inference Engines:** base **TensorRT 10.3.0.30** on CUDA 12.6.11 / cuDNN 9.3.0, verified by an end-to-end engine build. **NVIDIA TensorRT Edge-LLM is real and this board is on its support matrix** — the earlier "no such product" note in this spec was a research error and is retracted; see the correction below. It is **not yet installed** (it is a local CMake source build, no wheel and no apt package). **TensorRT-LLM is a different project**, and that one is genuinely not installed and has no Tegra aarch64 wheel. The VLM runtime for the first Stage G gate is **llama.cpp + GGUF**, with TensorRT Edge-LLM tracked as the higher-performance successor. Voice models are validated in their own runtimes first; TensorRT export stays a later, model-specific experiment rather than an assumed capability. Full inventory and smoke-test evidence: [Stage G1 notes](docs/bringup/07-tensorrt-g1.md); Edge-LLM evaluation: [07b notes](docs/bringup/07b-tensorrt-edge-llm.md).

  > **Correction, 2026-08-26.** A previous Stage G1 inventory concluded that "there is no NVIDIA product called 'TensorRT Edge-LLM'" and this spec was edited to match. **That conclusion was false and this spec's original wording was correct.** [`NVIDIA/TensorRT-Edge-LLM`](https://github.com/NVIDIA/TensorRT-Edge-LLM) is an active Apache-2.0 NVIDIA project (latest release **v0.10.0**, 2026-08-12) providing a C++ inference runtime for LLMs and VLMs on Jetson, DRIVE, and DGX Spark. Its v0.10.0 Official Support Matrix lists **Jetson Orin / JetPack 6.2+ / CUDA 12.6** as `Compatible` with "FP16, INT8, and INT4 only"; **Jetson Orin Nano 8 GB** is one of five benchmarked platforms; **Qwen2.5-VL-3B-Instruct** and **Qwen2.5-VL-3B-Instruct-AWQ** are both in the supported-models matrix; and **INT4 AWQ** is declared valid on all platforms while NVFP4 requires SM100+. The G1 error was to search for `tensorrt_llm`, find nothing, and generalise that to the spec's runtime — but the two are different projects, and Edge-LLM's absence from disk is simply its pre-install state. Everything else G1 measured stands.
